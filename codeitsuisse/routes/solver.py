...
from codeitsuisse.routes.game_state import GameState
import numpy as np
import time
        ...
        while nodes.qsize() and epochs <= self.__max_iter:
            ...
            if cur_state == self.__goal_state.flatten().tolist():
                self.__summary = str("A* took " + str(
                    ...
                    np.round(time.clock() - start_time, 4)) + " seconds to reach the desired solution.")
                while cur_node.get_parent():
                    self.__path.append(cur_node)
                    self.__answer.append(visited_nodes)
